...
export function UpdateNode({ node }: EditNodeProps) {
    ...
    const [nameOrUrl, setNameOrUrl] = useState(node.url ?? "");
    const [note, setNote] = useState(node.note ?? "");
    const [urlPreviewDescription, setUrlPreviewDescription] = useState(node.urlPreviewDescription ?? "");
    const [urlPreviewImageUrl, setUrlPreviewImageUrl] = useState(node.urlPreviewImageUrl ?? "");
    ...
    const update = () => {
        updateMutation.mutate({
            name, url: nameOrUrl, urlPreviewDescription, urlPreviewImageUrl, note, id: node.id
        });
        onClose();
    }
    ...
    return (
        ...
        <ModalHeader className="flex flex-col gap-1">Edit Item</ModalHeader>
              <ModalBody>
                ...
                <input
                  type="text"
                  placeholder="urlPreviewImageUrl"
                  value={urlPreviewImageUrl}
                  onChange={(e) => setUrlPreviewImageUrl(e.target.value)}
                  className="w-full rounded-full px-4 py-2 text-black"
                />
              </ModalBody>
              <ModalFooter>
